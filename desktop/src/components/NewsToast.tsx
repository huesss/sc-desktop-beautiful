    ...
    <Dialog.Root open={modalOpen} onOpenChange={setModalOpen}>
      ...
      >
        <Dialog.Trigger asChild>
          <button
            type="button"
            className={`relative flex w-[340px] cursor-pointer items-start gap-3.5 rounded-lg border bg-[#141414]/90 px-4 py-3.5 text-left transition-all duration-300 ease-[var(--ease-apple)] ${border} ${glow} hover:bg-[#1e1e24]/95 hover:scale-[1.01]`}
          >
            {}
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1 text-[12px] leading-relaxed text-[#ffffff99] line-clamp-2">
        ...
        <button
          ...
          className="absolute right-2 top-2 z-10 flex size-6 items-center justify-center rounded-lg bg-white/0 text-white/0 transition-all group-hover:bg-white/5 group-hover:text-[#ffffff99] hover:!bg-white/[0.1] hover:!text-[#ffffff99]"
        >
          <X size={12} />
        </button>
      </div>
      ...
      {}
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-[9999] bg-black/55 animate-in fade-in duration-200" />
        <Dialog.Content
          className={`fixed left-1/2 top-1/2 z-[10000] w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-lg border bg-[#141414]/95 shadow-[0_8px_64px_rgba(0,0,0,0.6)] overflow-hidden animate-in fade-in zoom-in-95 duration-200 ${mBorder}`}
        >
          {}
          <div className="flex items-center justify-between px-5 pt-5 pb-3">
            ...
            <Dialog.Close className="flex size-7 cursor-pointer items-center justify-center rounded-lg bg-white/[0.05] text-[#ffffff99] transition-colors hover:bg-white/[0.1] hover:text-[#ffffff99]">
              <X size={14} />
            </Dialog.Close>
          </div>
          ...
          <div className="px-5 pb-4">
            <p className="text-[13px] leading-relaxed text-[#ffffff99] whitespace-pre-line">
              {t(item.bodyKey)}
            </p>
          </div>
          ...
          <div className="flex gap-2 px-5 pb-5">
            <Dialog.Close className="flex-1 cursor-pointer rounded-xl bg-white/[0.05] py-2.5 text-[13px] font-medium text-[#ffffff99] transition-colors hover:bg-white/5">
              {t('news.dismissOnce')}
            </Dialog.Close>
            <button
              type="button"
              onClick={handleDismissForever}
              className="flex-1 cursor-pointer rounded-xl bg-white/[0.08] py-2.5 text-[13px] font-medium text-[#ffffff99] transition-colors hover:bg-white/[0.12]"
            >
