  ...
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-left disabled:opacity-50 ${
        containsAll ? 'bg-[#141414]' : 'hover:bg-white/5'
      }`}
    >
      <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0 ring-1 ring-white/10 bg-white/[.03]">
        {cover ? (
      ...
      <div className="flex-1 min-w-0">
        <p className="text-[13px] font-medium text-[#ffffff99] truncate">{playlist.title}</p>
        <p className="text-[11px] text-[#ffffff99]">{fc(playlist.track_count)} tracks</p>
      </div>
      {containsAll ? (
        ...
        <span className="shrink-0 text-[10px] font-semibold px-2 py-1 rounded-full bg-white/[0.06] text-[#ffffff99] border border-white/10">
...
const CreatePlaylistForm = React.memo(function CreatePlaylistForm({
  ...
  return (
    <div className="px-3 pb-3">
      <div className="rounded-xl bg-white/[.03] border border-white/10 p-3 space-y-2.5">
        <input
          ref={inputRef}
          type="text"
          value={name}
          ...
          className="w-full bg-[#141414] text-[13px] text-white placeholder:text-[#ffffff99] px-3 py-2 rounded-lg outline-none border border-white/10 focus:border-accent/30 transition-colors"
          autoFocus
...
export const AddToPlaylistDialog = React.memo(function AddToPlaylistDialog({
  ...
  return (
    <Dialog.Root open={open} onOpenChange={handleOpenChange}>
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/60 z-[70] animate-fade-in" />
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[70] w-[380px] max-h-[70vh] rounded-lg glass border border-white/10 shadow-2xl animate-fade-in-up flex flex-col">
          {}
          <div className="flex items-center justify-between px-5 pt-5 pb-3">
            <Dialog.Title className="text-[15px] font-bold text-white flex items-center gap-2">
              <ListPlus size={18} />
            ...
            <Dialog.Close className="w-7 h-7 rounded-lg flex items-center justify-center text-[#ffffff99] hover:text-[#ffffff99] hover:bg-white/5 transition-all">
              <X size={14} />
            </Dialog.Close>
          </div>
          ...
          {showCreate ? (
            ...
            <div className="px-3 pb-2">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/5 transition-all duration-200 text-left"
              >
          ...
          {}
          <div className="h-px bg-[#141414] mx-5" />
          ...
          <div className="px-3 py-2 pb-4 overflow-y-auto flex-1 min-h-0">
            {isLoading ? (
              ...
              <div className="py-10 text-center text-[13px] text-[#ffffff99]">
