...
export default function YMImportFloatingStatus() {
  ...
  return (
    <div className="pointer-events-none fixed top-5 right-5 z-[70] animate-fade-in-up">
      <div className="relative min-w-[220px] overflow-hidden rounded-lg border border-white/[0.14] bg-black/25 px-4 py-3 shadow-[0_16px_48px_rgba(0,0,0,0.3)] ">
        ...
        <div className="relative space-y-2">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#ffffff99]">
          ...
          <div className="relative h-2.5 overflow-hidden rounded-full bg-white/[0.08] ring-1 ring-white/10">
            <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(255,255,255,0.1),rgba(255,255,255,0.02))]" />
            <div
              className="absolute inset-y-0 left-0 rounded-full bg-[linear-gradient(90deg,rgba(255,120,64,0.72),rgba(0,168,67,1)_58%,rgba(255,185,120,0.92))] transition-[width] duration-500 ease-out"
              style={{ width: `${pct}%` }}
            />
            <div
              className="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full border border-white/45 bg-white shadow-[0_0_10px_rgba(255,255,255,0.85),0_0_18px_rgba(0,168,67,0.7)] transition-[left] duration-500 ease-out"
              style={{ left: glowLeft }}
            />
          </div>
